...
impl<T: FitnessEvaluator> GlobalState<T> {
    ...
    pub fn evaluated_is_sane(&self) -> bool {
        self.life.is_some() && self.thread_count.as_ref().unwrap().load(DEFAULT_ORDERING) == 0 &&
...
impl<T: FitnessEvaluator> PartialOrd for GlobalState<T> {
    fn partial_cmp(&self, other: &GlobalState<T>) -> Option<cmp::Ordering> {
        ...
        if (self.fitness.as_ref().unwrap().load(DEFAULT_ORDERING) as u64) <
           (other.fitness.as_ref().unwrap().load(DEFAULT_ORDERING) as u64) {
            Some(Less)
        } else if self.fitness.as_ref().unwrap().load(DEFAULT_ORDERING) as u64 >
           other.fitness.as_ref().unwrap().load(DEFAULT_ORDERING) as u64 {
            Some(Greater)
        } else {
            Some(Equal)
